...
const taskXCoordinate = (xDate: Date, dates: Date[], columnWidth: number) => {
  ...
  const firstDate = dates[index] || new Date();
  const secondDate = dates[index + 1] || new Date();

  const remainderMillis = xDate.getTime() - firstDate.getTime();

  const percentOfInterval =
    remainderMillis / (secondDate.getTime() - firstDate.getTime());
