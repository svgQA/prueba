...
function MapLibrePointsMap<T extends Point>({
  ...
  const mapStyle: string | maplibregl.StyleSpecification = {
    ...
    layers: [{ id: "carto-voyager", type: "raster", source: "carto", minzoom: 0, maxzoom: 19 }],
  }
  ...
  useEffect(() => {
    ...
    if (userLocation && points.length === 0) {
      ...
      pulseEl.className = "absolute w-12 h-12 bg-blue-400 rounded-full opacity-30 animate-ping"
  ...
  const havePositionsChanged = (prevPoints: T[], currentPoints: T[]): boolean => {
  ...
  useEffect(() => {
    ...
    const positionsChanged = havePositionsChanged(prevPointsRef.current, points)
    ...
    points.forEach((point) => {
      ...
      const popupContent = renderPopupContent ? renderPopupContent(point) : defaultRenderPopupContent(point)
    ...
    if (markersRef.current.size === points.length && points.length > 0 && markersRef.current.size <= 5) {
  ...
  }, [points, markerColor, renderPopupContent, onMarkerClick, fitBoundsOptions])
  ...
  return (
    <div className="relative w-full">
      ...
      <div className="absolute bottom-2 right-2 bg-white px-3 py-1.5 rounded shadow-sm text-xs font-medium text-gray-700 flex items-center gap-1.5">
        ...
        {locationError && points.length === 0 && <span className="ml-2 text-red-500">Error de ubicación</span>}
      </div>
    </div>
  )
}
