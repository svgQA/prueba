            ...
            {isViewerOpen && (
                ...
                >
                    <img
                        src={src}
                        alt='image-viewer'
                        style={{ maxHeight: '80vh', maxWidth: '90vw', borderRadius: 8, boxShadow: '0 2px 16px #0008' }}
                        onClick={e => e.stopPropagation()}
                    />
                    <button onClick={closeImageViewer} style={{ position: 'absolute', top: 24, right: 32, fontSize: 32, color: '#fff', background: 'none', border: 'none', cursor: 'pointer', zIndex: 10001 }}>&times;</button>
                </div>
            )}
        </>
    );
