...
export const Group = <T,>({ table, className = "" }: IGroupProps<T>) => {
  ...
  const selectedColumnName = useMemo(
    () => (currentGroup ? groupableColumns.find((col) => col.id === currentGroup)?.columnDef.header : ""),
    [currentGroup, groupableColumns],
  )

  const displayText = useMemo(
    () => (currentGroup && typeof selectedColumnName === "string" ? selectedColumnName : ""),
    ...
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      >
        <svg className="w-4 h-4 text-gray-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <path d="M3 4h18M3 12h12M3 20h6" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
