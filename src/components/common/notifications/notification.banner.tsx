...
const NotificationBanner = forwardRef<{ startBannerAnimation: () => void }, INotificationBannerProps>(({ message, reload, color = 'primary' }, ref) => {
            ...
            {notificationMemo > 0 && (
                <div className='ml-3 relative'>
                    <FloatBadge label={notificationMemo || '0'} color={`bg-${color}`}>
                        ...
                        >
                            <span className={`text-sm text-${color} pr-2`}>
                                {t(message)}
                            </span>
                        </div>
                    </FloatBadge>
