...
export const SiteCreatePage: FunctionComponent = () => {
  ...
  const loadAllPlaces = useCallback(async () => {
    ...
    if (req.getStatus()) {
      ...
      places.value = allPlaces.value.filter((p: any) => p.type === 'RESIDENTIAL');
    }
  }, []);
  ...
  const loadClientPlaces = useCallback(async (clientId: number) => {
    ...
    console.log(req)
    if (req.getStatus()) {
      places.value = normalizePlaces(req.getMany()).filter((p: any) => p.type === 'INDUSTRIAL');
      return;
    }
    ...
    initialValues.value = {
      ...
      clientCompanyId: (model as any)?.clientCompanyId
        ? {
            ...
            label: (model as any).clientCompany?.name ?? String((model as any).clientCompanyId),
          }
        : undefined,
      // NO TOCAR user
      userId: (model as any)?.userId
        ? { value: (model as any).userId, label: (model as any).user?.name ?? String((model as any).userId) }
        : undefined,
    };
  ...
  const onSubmit = async (model: FormData) => {
    ...
    if (selectedType === 'OFFICE') {
      ...
      if (!isIndustrialOpt(model.placeId)) {
        ToastManager.error('Debes seleccionar un Place INDUSTRIAL del cliente.');
        return;
      }
    } else {
      if (isIndustrialOpt(model.placeId)) {
        ToastManager.error('Para Casa/Apartamento el Place debe ser RESIDENTIAL.');
        return;
      }
    }
    ...
    const payload: ISiteCreate = {
      ...
      ...(selectedType === 'OFFICE' ? { clientCompanyId: Number(model.clientCompanyId!.value) } : {}),
      // NO TOCAR
    ...
    go({ to: '/trybook/sites', label: 'sites', id: 'trybook:sites:state', base: 'setting' });
  };
    ...
    <Section className="p-4 space-y-2 max-h-[67vh] overflow-y-auto vox-scroll-design">
      <Form
        ...
        render={({ handleSubmit, form, submitting, pristine }) => {
          ...
          const placeDisabled = isOffice ? !values.clientCompanyId?.value : false;

          return (
            <form onSubmit={handleSubmit} className="space-y-6" id="form-site-create">
              <StatusButton
              ...
              <div className="grid grid-cols-4 gap-2">
                ...
                <div className="col-span-2">
                  <Field<string> name="block">
                    {({ input, meta }) => (
                      <Input {...input} placeholder="p_block" label="l_block" type="text" meta={meta} />
                    )}
                  </Field>
                </div>
                  ...
                  <Field<string> name="houseNumber" validate={required}>
                    {({ input, meta }) => (
                      <Input
                        ...
                        label={isOffice ? 'Código/Número de oficina' : 'l_houseNumber'}
                        type="text"
                        meta={meta}
                      />
                    )}
