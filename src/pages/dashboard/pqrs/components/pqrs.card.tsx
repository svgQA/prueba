...
export const PqrsCards = ({
  ...
  return (
    ...
    >
      <div className='flex items-center justify-between text-[11px] uppercase tracking-wide text-gray-text-light dark:text-t-dark'>
        ...
        <span className='font-bold px-2 py-1 bg-teal-700 rounded-md'>
          {String(pqrs.id)}
        </span>
      </div>
      ...
      <div className='flex items-start justify-between gap-3'>
        <div class='flex-1 min-w-0 space-y-1.5'>
          ...
          <div class='flex items-center flex-wrap gap-2 text-[11px] text-gray-text-light dark:text-t-dark'>
            ...
            {pqrs?.contract && (
              <span class='px-2 py-0.5 rounded-full bg-ternary'>
                {pqrs?.contract}
              </span>
            )}
            {area && area.name && (
      ...
      {/* Metadata Grid */}
      <div className='grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-gray-text-light dark:text-t-dark'>
        {pqrs?.startDate && (
        ...
        {pqrs?.contactEmail && (
          <div class='flex items-center gap-1.5'>
            <span>📧</span>
            <TextEllipsis text={pqrs.contactEmail} maxWidth='100%' lines={1} />
          </div>
        )}
      </div>
      ...
      {(tags.length > 0 || pqrs?.resources || expirationStatus) && (
        <div class='flex flex-wrap gap-1.5 pt-2 border-t border-gray-border/60 dark:border-gray-border/30'>
          {tags.map((tag, idx) => (
            <span
              key={idx}
              class='px-2 py-0.5 bg-b-light text-white text-xs rounded-full dark:bg-b-dark'
            >
              #{String(tag)}
            </span>
          ))}
