...
const ReportInfo: React.FC<ReportInfoProps> = ({ reports: directReports, data, onViewDetails }) => {
      ...
      <div className="divide-y divide-gray-200">
        {reports.map((report) => {
          ...
          const requestDate = report.requestDate
            ? DateUtils.dateToFrontend(report.requestDate, { time: true, format: 'DD/MM/YYYY HH:mm' })
            : '—';
          const receivedDate = DateUtils.dateToFrontend(report.createdAt, { time: true, format: 'DD/MM/YYYY HH:mm' });
          const updatedDate = DateUtils.dateToFrontend(report.updatedAt, { time: true, format: 'DD/MM/YYYY HH:mm' });
            ...
            <React.Fragment key={report.id}>
              <div className="grid grid-cols-12 gap-x-4 items-center py-3 text-sm">
                  ...
                  {isRequested && (
                    <>
                      <p className="leading-tight">
                        <span className="font-semibold">Solicitud:</span> {requestDate}
                      </p>
                      <p className="leading-tight">
                        <span className="font-semibold">Recibido:</span> {receivedDate}
                      </p>
                    </>
                  )}
                  <p className="leading-tight">
                    <span className="font-semibold">Reporte:</span> {updatedDate}
                  </p>
                </div>
                ...
                <div className="col-span-2">
                  {hasAttachments ? (
                    ...
                    <p className="text-sm">
                      <span className="font-semibold">Formulario:</span> {report.form?.title}
                    </p>
                  ) : null}
                </div>
              ...
              {isExpanded && !hasAttachments && hasForm && (
                <div className="p-4 bg-gray-50 grid grid-cols-4 items-center gap-x-4">
                  <p className="text-sm">
                    <span className="font-semibold">Título:</span> {report.form?.title}
                  </p>
                  <p className="text-sm">
                    <span className="font-semibold">Categoría:</span> {report.form?.category ?? "Sin categoría"}
                  </p>
                  <p className="text-sm">
                    <span className="font-semibold">Descripción:</span> {report.form?.description}
                  </p>
