          ...
          <div key={index} className="grid grid-cols-12 gap-4 items-center">
            ...
            <div className="col-span-1">
              {report.status === "Solicitado" ? (
                <span className={`vox-icon size-sm vx-icon-${report.icon} !text-secondary`}></span>
              ) : (
                <span className={`vox-icon size-sm vx-icon-${report.icon} !text-error`}></span>
              )}
            </div>
            ...
            <div className="col-span-3">
              <p className="text-xs text-t-light-dark">
                Solicitud: {report.requestDate.split(" ")[0]} {report.requestDate.split(" ")[1]}
              </p>
              <p className="text-xs text-t-light-dark">
                Reporte: {report.reportDate.split(" ")[0]} {report.reportDate.split(" ")[1]}
              </p>
            </div>

            {/* Descripción */}
            <div className="col-span-3">
              <p className="text-xs text-t-light-dark truncate">{report.description}</p>
            </div>

            {/* Comentarios */}
            <div className="col-span-1 text-center">
              <span className="inline-flex items-center px-3 py-1 rounded-full bg-primary-opacity text-primary text-xs">
                <span className="vox-icon vx-icon-239 !text-primary mr-1"></span> {report.comments}
              </span>
            </div>

            {/* Ver detalles */}
            <div className="col-span-2 text-right">
              <a href={report.detailsLink} className="text-primary text-xs flex items-center justify-end">
                Ver detalles 
